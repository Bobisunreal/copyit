...
function replacebots(name bottype)
{
  // replace the bots with fake bot decoration , as a workaround for a bug in jsaver
    local actor act;
    ...
    local texture tex;
    local mesh meshtemp;
    local string temptag;
    local float tempscale;
    local name aminmator;
    local string namly;
    ...
    foreach AllActors(class'actor', act)
    {
     if (act.isa(bottype) )
     {
       temploc = act.location;       temprot = act.rotation;
       tex = act.skin;               aminmator = act.AnimSequence;
       tempscale = act.drawscale;    meshtemp = act.mesh;
       namly= string(act.name);
       act.destroy();
       actNewClass = class<actor>( DynamicLoadObject("UnrealI.dice", class'class' ) );
       if (actNewClass != none) // not neccecary...
       {
         actthing=Spawn(actNewClass,,,temploc,temprot);
          if (actNewClass != none) // mabye...
          {
           actthing.drawscale=tempscale;
           actthing.skin =tex;
           actthing.mesh = meshtemp;
           actthing.AnimSequence = aminmator;
           log("Replaced bot "$namly $ " with  mesh " $string(meshtemp) $ " and skin  " $string(tex));
           }else{
           log("failed to spawn replacement bot decoration");
           }
        }
      }
    }
    ...
    local trigger tgr;
      ...
      { // messages
        ...
        foreach AllActors(class'actor', act)
        {
         ...
         {
          if(act.tag == 'Portal1MSG'){specialevent(act).message ="1";};
          if(act.tag == 'Portal2MSG'){specialevent(act).message ="2";};
          if(act.tag == 'Portal3MSG'){specialevent(act).message ="3";};
          if(act.tag == 'Portal4MSG'){specialevent(act).message ="4";};
          if(act.tag == 'Portal5MSG'){specialevent(act).message ="5";};
         }
        ...
        if(Level.Title == "Firedrop")
      { //(bad playerstart).
        Log("Fixing Firedrop...",'mapfix');
        templocation.X = -876.000000;   templocation.Y = -384.000000;  templocation.Z = -88.000000;
       actNewClass = class<actor>( DynamicLoadObject("JCoopZ1SSF.jzPlayerStart", class'Class' ) ); actthing=Spawn(actNewClass,,,templocation);
       actNewClass = class<actor>( DynamicLoadObject("xcoop.xPlayerStart", class'Class' ) );  actthing=Spawn(actNewClass,,,templocation);

      };

      if(Level.Title == "Alien Level")
      { //(bad playerstart).
        Log("Fixing Alien Level...",'mapfix');
        templocation.X = -737.000000;   templocation.Y = 1348.000000;  templocation.Z =   -60.000000;
        actNewClass = class<actor>( DynamicLoadObject("JCoopZ1SSF.jzPlayerStart", class'Class' ) ); actthing=Spawn(actNewClass,,,templocation);
       actNewClass = class<actor>( DynamicLoadObject("xcoop.xPlayerStart", class'Class' ) );  actthing=Spawn(actNewClass,,,templocation);
      ...
      { // level has a cheatable edge fenceing
       ...
       // not needed since a new map revision
  ...
  function stargate()
 {
    ...
    local mover mvr;                 // movers
      ...
      if(Level.Title =="Level  2")
      {
          Log("--------Start map patch---------");
          replacebots('MaleThree');
          // easter egg teleporters
          actNewClass = class<actor>( DynamicLoadObject("copyit.lssg2easteregg", class'Class' ) );
          actthing=Spawn(actNewClass,,,vect(152.537811,-289.813232,-915.900024));
          // easter egg teleporters return trip
          actNewClass = class<actor>( DynamicLoadObject("copyit.lssg2easteregg1", class'Class' ) );
          actthing=Spawn(actNewClass,,,vect(216.164505,-291.051086,-915.900024));
          actNewClass = class<actor>( DynamicLoadObject("unreali.superhealth", class'Class' ) );
          actthing=Spawn(actNewClass,,,vect(361.768341,-291.051086,-915.900024));
          Log("Spawned easter eggs");
           foreach AllActors(class'actor', act)
           {

              if (act.isa('specialevent') )
              {
                if(act.tag == 'Text1') { specialevent(act).message ="Present";};
                if(act.tag == 'Text2') { specialevent(act).message ="MY HEAD .. WHAT TO SEM Yesterday all drank";};
                if(act.tag == 'Text3') { specialevent(act).message ="HOW MUCH HELL is it anyway ";};
                if(act.tag == 'Text4') { specialevent(act).message ="That'll be the day ";};
                if(act.tag == 'Text5') { specialevent(act).message ="Good thing I can get some sleep at the base";};
              }

           }
        Log("Captions patched...");


        actNewClass = class<actor>( DynamicLoadObject("JCoopZ1SSF.jzplayerstart", class'Class' ) );
        actthing=Spawn(actNewClass,,,vect(-268.605164,-9.675980,-852.900024)); // switchtrigger
        Log("PlayerstRT  patched...");
       }

      if(Level.Title =="Level 3")
      {
           replacebots('Bots');
           foreach AllActors(class'actor', act)
           {
              ...
              if (act.isa('specialevent') )
              {
                 if(act.tag == 'Text1') { specialevent(act).message ="WHERE TO HERE TODAY wonder you fly ..........";};
                 if(act.tag == 'Text2') { specialevent(act).message ="SG3 She should be back, so where SOU";};
                 if(act.tag == 'Text3') { specialevent(act).message ="I should go ASK WHAT IS THE CAPTAIN S SG3, somehow I do not think so.";};
                 if(act.tag == 'Text4') { specialevent(act).message ="I see that you smugglers got caught";};
              }
      ...
      if(Level.Title =="Level 5")
      {
           ...
           foreach AllActors(class'actor', act)
           {
              ...
              if (act.isa('specialevent') )
              {
                 if(act.tag == 'Text1') { specialevent(act).message ="HMM what is this homestead? ";};
                 if(act.tag == 'Text2') { specialevent(act).message ="We hope that here you will find someone on SG3 ";};
                 if(act.tag == 'Text3') { specialevent(act).message ="I'm just the prayers :-)";};
                 if(act.tag == 'Text4') { specialevent(act).message ="WHERE THERE MAJ HELL nějakýho rulers";};
                 if(act.tag == 'Text5') { specialevent(act).message ="FISH TED does not taste ..............";};
                 if(act.tag == 'Text6') { specialevent(act).message ="HOW THESE animals the ASI CHUTNAJ?";};
                 if(act.tag == 'Text7') { specialevent(act).message ="NO BOOKING TEDA look great ................";};
                 if(act.tag == 'Text8') { specialevent(act).message ="Maybe I'd read IO TU meaning of life ";};
              }
           }
       }

      if(Level.Title =="Level6")
      {
           ...
           foreach AllActors(class'actor', act)
           {
              ...
              if (act.isa('specialevent') )
              {
                 if(act.tag == 'kopkavs01') { specialevent(act).message ="HELL WAS THAT SPIRIT??!!";};
                 if(act.tag == 'kopkavs02') { specialevent(act).message ="I'll try to set him free";};
                 if(act.tag == 'kopkavs03') { specialevent(act).message ="AID spirit is rewarding";};
      ...
      if(Level.Title =="Level7")
      {

         // the pickups and stuff
      }


      if(Level.Title =="LEVEL7b")
      {
              if (act.isa('specialevent') )
              {
                 if(act.tag == 'SpecialEvent06') { specialevent(act).message ="Hmmmm here already is Cooler";};
                 if(act.tag == 'SpecialEvent07') { specialevent(act).message ="So that's why there is such WINTER OR SHOULD zeby smash CONDITIONING?";};
                 if(act.tag == 'SpecialEvent08') { specialevent(act).message ="SO this the way to sure to draw fiery RIVER GOD :-)";};
                 if(act.tag == 'SpecialEvent09') { specialevent(act).message ="BUH? Old wives' tales, AZ nějakýho so maybe we'll see";};
                 if(act.tag == 'SpecialEvent05') { specialevent(act).message ="HELL again some ghosts";};
              }
        ...
        { // All fixes for  sggl:Level8
           ...
           foreach AllActors(class'actor', act)
           {
            ...
            if (act.isa('brute') ) { act.tag = 'copy';} // dont let anything copy
             ...
             if (act.isa('specialevent') )
              {
              if(act.tag == 'prvnn01')  { specialevent(act).message ="Entrance to the chapel is free";};


             if(act.tag == 'text10')  { specialevent(act).message ="Sometimes SG3 live ?";};
             if(act.tag == 'text10')  { specialevent(act).message ="Sometimes SG3 live ?";};
              }
