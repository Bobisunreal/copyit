...
var (any) config string disableinmaps[50];
...
event BeginPlay()
{
      ...
      SetTimer(10.0, true); // time deley to avoid the lockup.
       ...
       if (bskipcow)   {  tempexclusions = tempexclusions $ "unreali.cow";   }
          ...
          {
            if (caps(disableinmaps[cunt]) ==  caps(level.outer))
             {
              log( "monstercopier spawned , but disabled for this map.");
              destroy();
             }
   ...
   For( Cunt = 0; Cunt <  9 ; Cunt++ )
          {
            ...
            {
               if (exceptionrules[cunt].map == string(level.outer))
               {
                 // found a rule.
